...
Для API key-запросов (X-API-Key) стандартный Go-транспорт работает нормально.
...
Kraube может использовать эти credentials напрямую через `NewClientFromClaude()`,
либо хранить свои в `~/.config/kraube/credentials.json`.
